...
### Violin plot for day of the week volume

Write about the plot here

``` {r days_of_week, echo = FALSE, fig.width=13, fig.height = 8, dpi = 300}

traffic_weekday <-
  traffic_converted %>%
  mutate(day_of_week = wday(date, label = TRUE, abbr = FALSE)) %>%
  group_by(date, day_of_week) %>%
  summarise(weekday_volume = sum(daily_volume), .groups = "drop")

ggplot(traffic_weekday, aes(x = day_of_week, y = weekday_volume, fill = day_of_week)) +
  geom_violin(alpha = 0.75) +
  scale_fill_viridis_d() +
  scale_y_continuous(labels = scales::label_comma(),
                     breaks = seq(0, 275000, 55000),
                     limits = c(0, 275000)) +
  labs(title = "Daily Traffic Volume by Day of Week",
       x = "",
       y = "Daily Volume") +
  theme_minimal() +
  theme(legend.position = "none",
